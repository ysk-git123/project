...
const ConnectionTest: React.FC = () => {
    ...
    const runTest = async (testName: string, testFn: () => Promise<{ data: unknown }>) => {
        setLoading(testName);
        try {
            const result = await testFn();
            Toast.show({
                content: `${testName}成功: ${JSON.stringify((result as { data: unknown }).data)}`,
                position: 'top',
            });
        } catch (error: unknown) {
            const errorMessage = error instanceof Error ? error.message : '未知错误';
            Toast.show({
                content: `${testName}失败: ${errorMessage}`,
                position: 'top',
            });
        } finally {
            setLoading(null);
